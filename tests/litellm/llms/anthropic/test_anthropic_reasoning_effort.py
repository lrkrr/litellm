...
class TestMapReasoningEffort:
    ...
    def test_none_string_returns_disabled_for_opus_4_6(self):
        """reasoning_effort='none' must send explicit {type: disabled} so we don't
        rely on omit-defaults-to-off (per Anthropic docs only Mythos Preview
        defaults to adaptive when thinking is unset, but being explicit is
    ...
    def test_none_string_returns_disabled_for_other_models(self):
        result = AnthropicConfig._map_reasoning_effort(
            reasoning_effort="none", model="claude-4-sonnet-20250514"
        )
        assert result == {"type": "disabled"}

    def test_disable_returns_disabled(self):
        result = AnthropicConfig._map_reasoning_effort(
...
class TestMapOpenAIParamsReasoningEffort:
    ...
    def test_disable_string_sends_explicit_disabled_thinking(self):
        """We send ``thinking: {type: "disabled"}`` instead of omitting the
        parameter so disable works correctly on models where omit-defaults-to-off
        is not guaranteed (e.g. future models, Mythos Preview)."""
        result = AnthropicConfig().map_openai_params(
    ...
    def test_disable_dict_sends_explicit_disabled_thinking(self):
        result = AnthropicConfig().map_openai_params(
            non_default_params={
